...
class MyCourseVieModel {
    var courseData: [Datum] = loadMockData()?.data ?? []
    var academicData:[AcademicData] = loadAcademicData()?.data ?? []
    var dataCourseListModel: [Datum] = loadMockData()?.data ?? []
    ...
    func filteredData(searchText: String = "") {
        ...
        //        case 1:
        //            filterArray = dataCourseListModel.filter({$0.id  })
        //        case 2:
        ...
        if searchText.isEmpty {
            ...
            //filterArray.compactMap({$0})
        } else {
            dataCourseListModel = filterArray.filter({ element in
                element.courseName?.lowercased().contains(searchText.lowercased()) == true || element.term?.lowercased().contains(searchText.lowercased()) == true || element.level?.lowercased().contains(searchText.lowercased()) == true
    ...
    func indexOf(course: Datum) -> Int? {
        dataCourseListModel.lastIndex(where: { $0.id == course.id && $0.term == course.term})
    }
    
    func updateSessionData(data: BelowModelData, course: Datum) {
        if let index = indexOf(course: course) {
            dataCourseListModel[index].sessionData = data
        }
    }
    
    func forApiIntegration(course: Datum,id: String = "",institutionCalendarId: String = "", courseType: String = "",level:String = "",term:String = "",rotation: String = "",rotationCount: Int = 0,completion: (() -> Void)? = nil ) {
        var orginalURL = "https://ecs-dsapi-staging.digivalitsolutions.com/api/v1/digiclass/course_session/userCourseSessionDetails/61a61789b1723a7f23346f70/\(id)?institutionCalendarId=\(institutionCalendarId)&type=student&level=\(level)&term=\(term)"
        if rotation == "yes" {
            orginalURL += "&rotationCount=\(rotationCount)"
        }
        ...
        request.allHTTPHeaderFields = [
            "AUthorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJJTlQwMDIyIiwiaWF0IjoxNjk0NTgzOTA4LCJleHAiOjE2OTQ2MTk5MDh9.aZ-NqgAzu_jJOTOlfYkpWKHIzOjXCIZ1tExBO5CMma8"
        ]
